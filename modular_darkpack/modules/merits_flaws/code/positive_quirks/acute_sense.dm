...
/datum/quirk/darkpack/acute_sense/add(client/client_source)
	...
	switch(sense)
		...
		if("sight")
			...
			if(sensitive_eyes)
				sensitive_eyes.flash_protect = max(sensitive_eyes.flash_protect -= 1)
		if("taste")
...
/datum/quirk/darkpack/acute_sense/remove()
	...
	switch(sense)
		if("hearing")
			...
			var/obj/item/organ/ears/sensitive_ears = human_holder.get_organ_slot(ORGAN_SLOT_EARS)
			if(sensitive_ears)
				sensitive_ears.damage_multiplier = initial(sensitive_ears.damage_multiplier)
		if("smell")
		...
		if("sight")
			...
			var/obj/item/organ/eyes/sensitive_eyes = human_holder.get_organ_slot(ORGAN_SLOT_EYES)
			if(sensitive_eyes)
				sensitive_eyes.flash_protect = initial(sensitive_eyes.flash_protect)
		if("taste")
